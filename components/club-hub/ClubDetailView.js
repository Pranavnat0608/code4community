...
export default function ClubDetailView({ club, slug }) {
  ...
  const checkMembership = async () => {
    ...
    try {
      const member = await isUserClubMember(user.uid, club.name);
      setIsMember(member);
    } catch (err) {
  ...
  const handleJoinClub = async () => {
    if (!user || !userData || !club) return;
    setMembershipLoading(true);
    setError("");
    try {
      const result = await joinClub(user.uid, userData.displayName, club.name);
      if (result.success) {
        setIsMember(true);
  ...
  const handleLeaveClub = async () => {
    ...
    setError("");
    try {
      const result = await leaveClub(user.uid, club.name);
      if (result.success) {
        setIsMember(false);
        setMessage("Successfully left club.");
        setTimeout(() => setMessage(""), 3000);
      } else {
        setError(result.message);
      }
              ...
              <>
                {isMember ? (
                  <button
                    ...
                    className="rounded-md border border-red-300 bg-red-50 px-3 py-1.5 text-sm font-semibold text-red-900 hover:bg-red-100 disabled:opacity-50"
                  >
                    {membershipLoading ? "Leaving..." : "Leave Club"}
                  </button>
                ) : (
                  <button
                    type="button"
                    ...
                    disabled={membershipLoading}
                    className="rounded-md border border-green-300 bg-green-50 px-3 py-1.5 text-sm font-semibold text-green-900 hover:bg-green-100 disabled:opacity-50"
                  >
                    {membershipLoading ? "Joining..." : "Join Club"}
                  </button>
                )}
              </>
            )}
